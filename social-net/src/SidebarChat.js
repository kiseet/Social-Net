...
import { Cancel, DoneOutlineOutlined, DonutLargeOutlined, DonutLargeRounded, Mic, PhotoAlbum, Search } from '@mui/icons-material';
                        ...
                        arr.push(<Link className="link" key={data.id} to={{
                            ...
                            >
                                <div className="avatar__container">
                                    <AvatarGroup style={{width: 45,height: 45}} src={`${data.photoURL ? data.photoURL : `https://avatars.dicebear.com/api/human/${data.id}.svg`}`} />
                                ...
                                <div className="sidebar__chat--info">
                                    ...
                                    <p style={{width: page.width <= 760 ? page.width - 126 : page.width * 0.315 - 126}}>
                                        {lastMessage?.message ? 
                                            <><PhotoAlbum style={{width: 19,height: 19}} /> <span style={{width: page.width <= 760 ? page.width - 150 : page.width * 0.315 - 150}}>{lastMessage.message}</span> </>
                                            : lastMessage?.audio ?
                                            <><Mic style={{width: 19,height: 19}} /><span style={{width: page.width <= 760 ? page.width - 150 : page.width * 0.315 - 150}}>{lastMessage.time}</span></>
                                            : lastMessage?.message === "" ?
                                            <><PhotoAlbum style={{width: 19,height: 19}} /> <span style={{width: page.width <= 760 ? page.width - 150 : page.width * 0.315 - 150}}>Photo</span> </>
                                        :lastMessage}
                                    </p>
                                </div>
    ...
    return (
        <div ref={sidebarChatContainer} className="sidebar__chat--container">
            ...
            <div className="loader__container scrollFetch">
                {scrollFetch ? <DoneOutlineOutlined /> : null}
            </div>
        </div>
    )
}
