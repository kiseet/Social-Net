...
import { DonutLarge, Pause, PlayArrow, PlayArrowOutlined, PlayForWorkRounded } from '@mui/icons-material';
		...
		<>
			<div className={`${sender ? "audioplayer" : "audioplayer audioplayer2"} ${audioPlayed ? "audioplayer__played" : ""}`}>
				{!mediaLoaded && !error ?
					<DonutLarge/>
				: isPlaying && !error? 
					<Pause className="pause" onClick={pause} /> 
				: !isPlaying && !error ?
					<PlayArrowOutlined onClick={play} />
				: 
					<PlayForWorkRounded onClick={loadAudioAgain} />
				}
				<div>
					<span 
						style={{
